...
if __name__ == "__main__":
    scipy = pd.read_csv(
        "examples/latent_tgas/scipy/scipy_dy_no_heating.csv", index_col=0
    )
    jax = pd.read_csv("examples/latent_tgas/jax/jax_dy_no_heating.csv", index_col=0)
    carbox = pd.read_csv("carbox/carbox_dy_no_heating.csv", index_col=0)
    ...
    jax.loc[:, molecules].plot(
        ax=ax, loglog=True, linestyle="--", label=None, legend=None
    )
    ax.set_prop_cycle(plt.cycler("color", plt.cm.tab20.colors))
    scipy.loc[:, molecules].plot(
        ax=ax, loglog=True, linestyle="dotted", label=None, legend=False
    )
    ...
    ax.axvline(1e10)
    ax.set_ylim(-1e5, 1e5)
    ax.set(xlabel="Time (s)", ylabel="Rate of change (cm$^{-3}$ s$^{-1}$)")
    fig.show()
    plt.show()
