...
from ..reactions import KAReaction, CRReaction, FUVReaction
import pandas as pd
...
class UMISTParser(BaseParser):
    ...
    def parse_network(self, filepath: str) -> Network:
        ...
        df = pd.read_csv(filepath, sep=':', comment='#', names=range(46), header=None).iloc[:, :18]
        
        # Convert to list format for compatibility with existing code
        reactions_data = df.values.tolist()
        # Convert to DataFrame for easier processing
        columns = [
            'reaction_number', 'reaction_type', 'reactant_1', 'reactant_2', 
            'product_1', 'product_2', 'product_3', 'product_4', 'stoich_reactant_1',
            'alpha', 'beta', 'gamma', 'tlow', 'thigh', 'uncertainty_flag',
            'source', 'reference_1', 'reference_2', 'notes'
        ]
    ...
    def parse_reaction(self, row) -> Optional[KAReaction]:
        ...
        try:
            ...
            reactants = self._parse_species_list(row["reactant_1"]) + self._parse_species_list(row["reactant_2"])
            products = self._parse_species_list(row["product_1"]) + self._parse_species_list(row["product_2"]) + \
                       self._parse_species_list(row["product_3"]) + self._parse_species_list(row["product_4"])
    ...
    def _parse_species_list(self, species_str: str) -> List[str]:
        ...
        if isinstance(species_str, str) and (not species_str or  species_str.strip() == ''):
            return []
