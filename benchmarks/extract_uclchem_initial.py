...
def extract_initial_conditions(
    network_name: str, results_dir: str = "results", gas_phase_only: bool = True
):
    ...
    for species, abundance in initial_row.items():
        ...
        if gas_phase_only:
            if (
                species.startswith("#")
                or species.startswith("@")
                or species in ["BULK", "SURFACE"]
            ):
                continue
    ...
    initial_abundances = dict(
        sorted(initial_abundances.items(), key=lambda x: x[1], reverse=True)
    )
...
def save_to_yaml(
    initial_abundances: dict, network_name: str, output_dir: str = "initial_conditions"
):
...
def main():
    ...
    parser.add_argument(
        "--output", default="initial_conditions", help="Output directory"
    )
