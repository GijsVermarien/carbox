...
def get_common_species(uclchem_df, carbox_df):
    ...
    uclchem_species = set(
        col for col in uclchem_df.columns if col != "time" and not col.endswith("_ice")
    )
...
def plot_comparison(uclchem_df, carbox_df, species, output_file, title):
    ...
    for i, idx in enumerate(top_indices):
        ...
        ax.loglog(
            uclchem_df["time"], uclchem_df[sp], "b-", label="UCLCHEM (gas)", linewidth=2
        )
        ax.loglog(
            carbox_df["time_years"], carbox_df[sp], "r--", label="Carbox", linewidth=3
        )
...
def compare_results(network_name: str, results_dir: str = "results"):
    ...
    species = [s for s in species if s in uclchem_df.columns and s in carbox_df.columns]
    ...
    carbox_bench_file = Path(results_dir) / "carbox" / f"{network_name}_benchmark.json"
    ...
    with open(perf_file, "w") as f:
        ...
        f.write(
            f"| **Total Time** | {ucl_time:.2f}s | {cbx_time:.2f}s | {speedup:.2f}× |\n"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Compare UCLCHEM and Carbox results")
