...
def load_results(
    results_dir: str, network_name: str
...
def get_common_species(uclchem_df: pd.DataFrame, carbox_df: pd.DataFrame) -> list[str]:
...
def interpolate_to_common_times(
    uclchem_df: pd.DataFrame, carbox_df: pd.DataFrame, species: list[str]
...
def compute_differences(
    uclchem_abund: np.ndarray, carbox_abund: np.ndarray, threshold: float = 1e-15
) -> np.ndarray:
    ...
    rel_diff[mask] = (
        np.abs(carbox_abund[mask] - uclchem_abund[mask]) / uclchem_abund[mask]
    )
...
def generate_statistics_report(
    ...
    with open(output_file, "w") as f:
        ...
        if len(valid_diffs) > 0:
            ...
            f.write(
                f"  Median relative difference: {np.median(valid_diffs) * 100:.2f}%\n"
            )
        ...
        f.write("-" * 40 + "\n")
        f.write(
            f"{'Species':<12} {'Mean %':<10} {'Max %':<10} {'Final UCLCHEM':<15} {'Final Carbox':<15}\n"
        )
        f.write("-" * 70 + "\n")
        ...
        for i, sp in enumerate(species):
            ...
            f.write(
                f"{sp:<12} {mean_diff:>9.2f} {max_diff:>9.2f} {final_ucl:>14.3e} {final_cbx:>14.3e}\n"
            )
...
def compare_abundances(
    ...
    return {
        ...
        "mean_rel_diff": float(np.mean(valid_diffs))
        if len(valid_diffs) > 0
        else np.nan,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Compare UCLCHEM and Carbox abundances"
    )
    parser.add_argument("--results", default="../results", help="Results directory")
    parser.add_argument("--network", required=True, help="Network name")
    parser.add_argument(
        "--output", default=None, help="Output directory (default: results/comparisons)"
    )
    ...
    stats = compare_abundances(
        args.results, args.network, args.output, verbose=not args.quiet
    )
