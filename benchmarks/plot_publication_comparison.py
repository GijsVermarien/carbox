...
tab10_colormap = colormaps["tab10"]
...
def plot_species_comparison(
    ...
    colors = tab10_colormap(np.linspace(0, 1, 10))
    ...
    species_handles = [
        Line2D(
            [0], [0], color=colors[i], linewidth=2.5, label=format_species_name(species)
        )
        for i, species in enumerate(species_left)
        if species in uclchem_df.columns and species in carbox_df.columns
    ]
    ...
    framework_handles = [
        Line2D([0], [0], color="gray", linestyle="-", linewidth=2.5, label="UCLCHEM"),
        Line2D([0], [0], color="gray", linestyle="--", linewidth=2.5, label="Carbox"),
    ]
    ...
    all_handles_right = (
        species_handles_right + [Line2D([0], [0], color="none")] + framework_handles
    )
...
def plot_publication_comparison(
    ...
    all_handles_left = (
        species_handles_left + [Line2D([0], [0], color="none")] + framework_handles
    )
    ...
    for i, species in enumerate(species_right):
        if species in uclchem_right.columns and species in carbox_right.columns:
            ...
            ax2.loglog(
                carbox_right["time_years"],
                carbox_right[species],
                color=color,
                linestyle="--",
                linewidth=2.5,
                alpha=0.9,
